...
void interpret_swap  ( struct vm_state* state )  {
    ...
    struct maybe_int64 tmp2 = stack_pop(&(state -> data_stack));
        stack_push(&(state -> data_stack), tmp1.value);
        stack_push(&(state -> data_stack), tmp2.value);
...
{ 
    struct maybe_int64 tmp = stack_pop(s);
    if(tmp.valid){
        stack_push(s, f(tmp.value));
    } 
}
...
{
    struct maybe_int64 tmp1 = stack_pop(s);
    struct maybe_int64 tmp2 = stack_pop(s);
    if(tmp1.valid && tmp2.valid){
        stack_push(s, f(tmp2.value, tmp1.value));
    } 
