...
struct stack stack_create( size_t size ){
    struct array_int arr = array_int_create(size);
    return (struct stack){ .count = 0, .data = arr};
};
...
stack_pop( struct stack* s ){
    ...
    (*s).count--;
    struct maybe_int64 ans = (struct maybe_int64){.value=*((*s).data.data+(*s).count), .valid=true};
